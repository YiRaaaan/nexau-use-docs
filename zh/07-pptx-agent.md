...
- **加新工具**（第 2 章 ExecuteSQL、第 4 章 write_todos）→ 让框架知道怎么"做新事情"
...
装好之后，在 `nexau-tutorial/` 下装 `pptxgenjs`：
...
这会创建一个 `node_modules/`。本章的智能体会从 `nexau-tutorial/` 跑 `node`，所以脚本能直接 `require("pptxgenjs")`。
...
打开 `nexau-tutorial/output/haidian_top10.pptx`（macOS 上 `open output/haidian_top10.pptx`、Linux 上 `xdg-open`、Windows 双击），应该能看到一份配色不糟、数据真实、SQL 可追溯的 PPT。
...
- 用 `write_todos` 把"3 个查询 + 1 个 PPT 生成"拆成 4 步追踪
- 生成一份多页 PPT
