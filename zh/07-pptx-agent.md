...
**TL;DR**:给 NL2SQL 智能体挂上 Anthropic 官方的 `pptx` Skill 和三个内置工具,它就能根据查询结果自动写 `pptxgenjs` 脚本、跑 `node`、生成一份配色不糟、数据真实、SQL 可追溯的 `.pptx`。`agent.yaml` 多 4 行,`system_prompt.md` 多一段,`bindings.py` 一行不动。

> **本章假设你**已经跟着第 1–6 章把 NL2SQL 智能体跑起来过。如果还没,先回到[第 1 章](./01-bash-nl2sql.md)。
...
```bash
cd NexAU
ln -s ~/.agents/skills/pptx nl2sql_agent/skills/pptx
```

验证:

```bash
ls -l nl2sql_agent/skills/pptx
...
打开 `nl2sql_agent/agent.yaml`,在 `tools:` 段加三个内置工具,在 `skills:` 段加 pptx:
...
模型现在多了一种"完成方式":除了"回答问题",还能"产出一份 PPT"。需要在 system prompt 里告诉它什么时候选哪种。打开 `nl2sql_agent/system_prompt.md`,在 Workflow 段后面加一段 Output Modes:
...
```bash
dotenv run uv run nl2sql_agent/start.py "给我做一份海淀区 TOP 10 注册资本企业的简报 PPT"
```
...
4. (可能)`read_file(path="nl2sql_agent/skills/pptx/pptxgenjs.md")` —— 对某个 API 不确定时回头读子文档
...
```bash
dotenv run uv run nl2sql_agent/start.py "给我做一份各专精特新等级企业数量 + 主营行业分布的简报"
```
