...
**TL;DR**：给企业数据分析 Agent 挂上 Anthropic 官方的 `pptx` Skill 和三个内置工具，它就能根据查询结果自动编写 `pptxgenjs` 脚本、执行 `node`、生成一份配色不糟、数据真实、SQL 可追溯的 `.pptx`。`agent.yaml` 多 4 项配置，`system_prompt.md` 多一段，`tools/execute_sql.py` 一行不动。

> **本章假设你**已经跟着第 1–6 章把企业数据分析 Agent 运行过。若尚未完成，请先回到[第 1 章](zh/01-bash-nl2sql.md)。
...
到第 5 章为止，智能体的输出始终是**一段文字 + 一段 SQL**。但业务方需要的并非如此，而是**一份可以直接发出的报告**：海淀区 TOP 10 简报、专精特新分布概览、季度融资榜的 PPT。

补充这部分能力，有两条路：

1. **手写一个 PPT 工具**：在 Python 中维护模板和填槽逻辑——脆弱、难维护、每出一种新报告都要改代码
2. **教模型自行编写 PPT 代码**：给它一个 Skill 讲解如何使用 `pptxgenjs`、再给它几个能写文件和执行 shell 的内置工具

第二条路就是本章内容。**整章核心变更只有 `agent.yaml` 多 4 项配置、`system_prompt.md` 多一段、再补两份 tool schema 文件——`tools/execute_sql.py` 一行不动。**
...
打开 `enterprise_data_agent/agent.yaml`，先把 `max_iterations` 从 `30` 调到 `80`：

```yaml
max_iterations: 80
```

PPT 生成的工具链很长——读 pptx Skill → 读表 Skill → 执行 SQL → 写 JS 文件 → 执行 node → 检查输出，一次完整的 Mode B 流程很容易消耗 20–30 轮迭代。保留原来的 `30` 会频繁触发 `Maximum iteration limit reached`。

然后在 `tools:` 段加三个内置工具，在 `skills:` 段加 pptx：
...
name: read_file
description: >-
  Reads and returns the content of a specified text file. If the file is large,
  the content will be truncated. The tool's response will clearly indicate if
  truncation has occurred and will provide details on how to read more of the
  file using the 'offset' and 'limit' parameters. Handles text files only.
input_schema:
  type: object
  properties:
    file_path:
...
│   ├── TodoWrite.tool.yaml
│   ├── WriteFile.tool.yaml          # 第 6 章新增
│   ├── ReadFile.tool.yaml           # 第 6 章新增
│   └── RunShellCommand.tool.yaml    # 第 1 章创建,第 6 章重新启用
└── skills/
    ├── enterprise_*/
    ├── industry*/
    ├── users/
...
真正需要手写的 Python 只有一个 `tools/execute_sql.py`（~270 行），其余全是声明式的 YAML 和 Markdown。6 章下来：

- **结构化、安全的工具**（第 2 章）
- **领域知识按需加载**（第 3 章）
- **多步任务规划**（第 4 章）
- **超长输出截断**（第 5 章）
- **跨四种 LLM 协议**（附录 [LLM 协议切换](llm-api-types.md)）
- **多输出模式 + 第三方 Skill 复用**（第 6 章）

剩下的工作不再是"框架问题"，而是**产品问题**——迭代 Skill、为特定客户调整色板、接入 tracing、将智能体部署到真正的 UI 后面。框架的职责已经完成。

## 制品下载

不想从头跟完 6 章？直接下载完整的最终产物：

- <a href="/enterprise_data_agent.zip" download>enterprise_data_agent.zip</a> —— 完整 agent（agent.yaml + system_prompt + tools/execute_sql.py + 全部 tool schema 和 skills，含 pptx）+ 示例数据库，解压后只需 `npm install pptxgenjs` + 配置 `.env` 即可运行
- <a href="/skills.zip" download>skills.zip</a> —— 全部 8 个 Skills（7 个领域 + pptx），可单独解压覆盖到已有项目
- <a href="/enterprise.sqlite" download>enterprise.sqlite</a> —— 示例数据库（7 张表，每表约 50 行）
...
- [第 3 章 · 写自己的 Skill](zh/03-skills.md) —— 若要为自己的业务领域编写工作流 Skill，可从该章的格式起步
