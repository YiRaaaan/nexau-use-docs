...
| **nexau-use-docs**（就是这份文档的仓库） | 文档 + 一个完整可跑的 `enterprise_data_agent/` 示例 + `enterprise.sqlite` 数据库 | 你只需要从这里下载两样东西：`enterprise.sqlite` 和（可选地）`enterprise_data_agent/` 整套作为参考 |

教程的写法是**手把手在 `NexAU/` 下建一个新的 `enterprise_data_agent/`**。如果你愿意一步步敲，就跟着第 1 章往下走;如果你想直接看成品，把 nexau-use-docs 仓库里的 `enterprise_data_agent/` 文件夹整个复制到 `NexAU/` 下也能跑——两者文件结构是一样的。
