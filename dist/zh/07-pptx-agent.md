...
**TL;DR**：给企业数据分析 Agent 挂上 Anthropic 官方的 `pptx` Skill 和三个内置工具，它就能根据查询结果自动编写 `pptxgenjs` 脚本、执行 `node`、生成一份配色不糟、数据真实、SQL 可追溯的 `.pptx`。`agent.yaml` 多 4 行，`system_prompt.md` 多一段，`bindings.py` 一行不动。
...
第二条路就是本章内容。**整章核心代码变更只有 `agent.yaml` 多 4 行 + `system_prompt.md` 多一段。**
...
`write_file`、`read_file`、`run_shell_command` 三个工具均复用 NexAU 内置实现（`binding` 指向 `nexau.archs.tool.builtin.*`），各自需要一份 schema 文件。`RunShellCommand.tool.yaml` 第 1 章已写好，这里补上另外两个。
